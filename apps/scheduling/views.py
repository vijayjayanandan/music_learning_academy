...
class SessionCreateView(TenantMixin, CreateView):
        ...
        if hasattr(request, "academy") and request.academy:
            ...
            if role not in ("owner", "instructor"):
                ...
                return HttpResponseForbidden(
                    "Only instructors and owners can create sessions."
                )
    ...
    def form_valid(self, form):
        ...
        if overlap:
            form.add_error(
                None, "This session overlaps with an existing scheduled session."
            )
            return self.form_invalid(form)

        session.room_name = generate_room_name(self.get_academy().slug, id(session))
        session.save()
...
class SessionDetailView(TenantMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["can_join"] = ctx["is_instructor"] or ctx["is_registered"]
...
class SessionEditView(TenantMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if hasattr(request, "academy") and request.academy:
            ...
            if role not in ("owner", "instructor"):
                ...
                return HttpResponseForbidden(
                    "Only instructors and owners can edit sessions."
                )
        return super().dispatch(request, *args, **kwargs)
    ...
    def get_success_url(self):
        return (
            ...
            else f"/schedule/session/{self.object.pk}/"
        )
...
class CancelSessionView(TenantMixin, View):
    def post(self, request, pk):
        ...
        if role not in ("owner", "instructor"):
            ...
            return HttpResponseForbidden(
                "Only instructors and owners can cancel sessions."
            )
        ...
        if request.htmx:
            return render(
                request,
                "scheduling/partials/_session_card.html",
                {
                    "session": session,
                },
            )
...
class RegisterForSessionView(TenantMixin, View):
    def post(self, request, pk):
        ...
        if request.htmx:
            return render(
                request,
                "scheduling/partials/_register_button.html",
                {
                    "session": session,
                    "is_registered": True,
                },
...
class MarkJoinedView(TenantMixin, View):
    def post(self, request, pk):
        ...
        session = get_object_or_404(LiveSession, pk=pk, academy=self.get_academy())
        SessionAttendance.objects.filter(session=session, student=request.user).update(
            status=SessionAttendance.AttendanceStatus.ATTENDED,
            joined_at=timezone.now(),
        )
        return render(
            request,
            "scheduling/partials/_attendance_status.html",
            {
                "status": "joined",
            },
        )
            ...
            events.append(
                {
                    ...
                    "extendedProps": {
                        ...
                        "instructor": s.instructor.get_full_name()
                        or s.instructor.email,
                        "status": s.status,
                    },
                }
            )
...
class BookSessionView(TenantMixin, View):
    ...
    def post(self, request):
        ...
        if existing:
            return self._error_response(
                request, "This slot is already booked for the selected date."
            )
...
class BookSessionSlotsView(TenantMixin, View):
    ...
    def get(self, request):
        ...
        if not instructor_id:
            return render(
                request,
                "scheduling/partials/_booking_slots.html",
                {
                    "slots": [],
                },
            )
...
class BookSessionConfirmView(TenantMixin, View):
    ...
    def post(self, request):
        ...
        try:
            ...
            return render(
                request,
                "scheduling/partials/_booking_error.html",
                {
        ...
        return render(
            request,
            "scheduling/partials/_booking_confirm.html",
            {
                "instructor": instructor,
                "slot": slot,
...
class RescheduleSessionView(TenantMixin, View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if is_student_own:
            hours_until = (
                self.session.scheduled_start - timezone.now()
            ).total_seconds() / 3600
    ...
    def get(self, request, pk):
        ...
        return render(
            request,
            "scheduling/reschedule_session.html",
            {
                "session": self.session,
            },
        )
    ...
    def _handle_student_reschedule(self, request):
        ...
        new_start = timezone.make_aware(datetime.combine(session_date, slot.start_time))
        new_end = timezone.make_aware(datetime.combine(session_date, slot.end_time))
    ...
    def _student_error(self, error_msg):
        ...
        return render(
            self.request,
            "scheduling/reschedule_session_student.html",
            {
                "session": self.session,
                "slots": slots,
                "error": error_msg,
    ...
    def _create_rescheduled_session(self, new_start, new_end, reason=""):
        ...
        SessionAttendance.objects.filter(session=self.session).update(
            session=new_session
        )
